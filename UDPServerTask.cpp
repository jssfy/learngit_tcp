...
namespace KIDD {
	...
	// this task is simply for one UDP server listening on a port
	void UDPServerTask::Run() {
		...
		for (;;) {
			...
			n = recvfrom(sockfd, &mesg, sizeof(mesg), 0, &temptr,
				     &temp);

			if (n < 0) {
				printf
				    ("recvfrom error in server.c with error \n");
				perror("recvfrom");
				exit(1);
			}
